...
try {
    ...
    $queryLabollene = "SELECT * FROM montecarlo_labollene ORDER BY time ASC LIMIT 10;";
    $stmtLabollene = $pdo->prepare($queryLabollene);
    $stmtLabollene->execute();
    $resultsLabollene = $stmtLabollene->fetchAll(PDO::FETCH_ASSOC);

    $queryLamairis = "SELECT * FROM montecarlo_lamairis ORDER BY time ASC LIMIT 10;";
    $stmtLamairis = $pdo->prepare($queryLamairis);
    $stmtLamairis->execute();
    $resultsLamairis = $stmtLamairis->fetchAll(PDO::FETCH_ASSOC);

    $queryMoissiere = "SELECT * FROM montecarlo_moissiere ORDER BY time ASC LIMIT 10;";
    $stmtMoissiere = $pdo->prepare($queryMoissiere);
    $stmtMoissiere->execute();
    $resultsMoissiere = $stmtMoissiere->fetchAll(PDO::FETCH_ASSOC);
    
    $queryRavindecoste = "SELECT * FROM montecarlo_ravindecoste ORDER BY time ASC LIMIT 10;";
    $stmtRavindecoste = $pdo->prepare($queryRavindecoste);
    $stmtRavindecoste->execute();
    $resultsRavindecoste = $stmtRavindecoste->fetchAll(PDO::FETCH_ASSOC);
...
<html lang="en">
    ...
    <div id="main-container" class="main-container">
        <div class="overlay">
            <h2>RALLYE MONTE CARLO</h2>

            <div class="stage-container">    
                <h3>LA BOLLENE-VESUBIE - COL DE TURINI 9.21km</h3> 
                <h5>(LA BOLLENE-VESUBIE - PEIRA CAVA 1st LEG)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsLabollene)): ?>
                ...
                <table class="time-table">
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsLabollene as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('montecarlo_labollene', 'LA BOLLENE-VESUBIE - COL DE TURINI', '9.21km', 'LA BOLLENE-VESUBIE - PEIRA CAVA 1st LEG')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('montecarlo_labollene', 'LA BOLLENE-VESUBIE - COL DE TURINI', '9.21km', 'LA BOLLENE-VESUBIE - PEIRA CAVA 1st LEG')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>LA MAIRIS 9.30km</h3> 
                <h5>(LA BOLLENE-VESUBIE - PEIRA CAVA 2nd LEG)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsLamairis)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsLamairis as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                        <td><?= htmlspecialchars($row["driver"]) ?></td>
                        <td><?= htmlspecialchars($row["class"]) ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('montecarlo_lamairis', 'LA MAIRIS', '9.30km', 'LA BOLLENE-VESUBIE - PEIRA CAVA 2nd LEG')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('montecarlo_lamairis', 'LA MAIRIS', '9.30km', 'LA BOLLENE-VESUBIE - PEIRA CAVA 2nd LEG')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>MOISSIERE 8.18km</h3> 
                <h5>(8.18km SAINT-LEGER-LES-MELEZES 1st LEG)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsMoissiere)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsMoissiere as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                        <td><?= htmlspecialchars($row["driver"]) ?></td>
                        <td><?= htmlspecialchars($row["class"]) ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('montecarlo_moissiere', 'MOISSIERE', '8.18km', 'SAINT-LEGER-LES-MELEZES 1st LEG')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('montecarlo_moissiere', 'MOISSIERE', '8.18km', 'SAINT-LEGER-LES-MELEZES 1st LEG')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>RAVIN DE COSTE BELLE 8.60km</h3> 
                <h5>(8.60km SAINT-LEGER-LES-MELEZES 2nd LEG)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsRavindecoste)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsRavindecoste as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                        <td><?= htmlspecialchars($row["driver"]) ?></td>
                        <td><?= htmlspecialchars($row["class"]) ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('montecarlo_ravindecoste', 'RAVIN DE COSTE BELLE', '8.60km', 'SAINT-LEGER-LES-MELEZES 2nd LEG')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('montecarlo_ravindecoste', 'RAVIN DE COSTE BELLE', '8.60km', 'SAINT-LEGER-LES-MELEZES 2nd LEG')">SHOW ALL</button>
            </div>
