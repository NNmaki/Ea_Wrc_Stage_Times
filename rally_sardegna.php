...
try {
    ...
    $queryLittichedda = "SELECT * FROM sardegna_littichedda ORDER BY time ASC LIMIT 10;";
    $stmtLittichedda = $pdo->prepare($queryLittichedda);
    $stmtLittichedda->execute();
    $resultsLittichedda = $stmtLittichedda->fetchAll(PDO::FETCH_ASSOC);

    $queryBortigiadas = "SELECT * FROM sardegna_bortigiadas ORDER BY time ASC LIMIT 10;";
    $stmtBortigiadas = $pdo->prepare($queryBortigiadas);
    $stmtBortigiadas->execute();
    $resultsBortigiadas = $stmtBortigiadas->fetchAll(PDO::FETCH_ASSOC);

    $queryMontemuvri = "SELECT * FROM sardegna_montemuvri ORDER BY time ASC LIMIT 10;";
    $stmtMontemuvri = $pdo->prepare($queryMontemuvri);
    $stmtMontemuvri->execute();
    $resultsMontemuvri = $stmtMontemuvri->fetchAll(PDO::FETCH_ASSOC);
...
<html lang="en">
    ...
    <div id="main-container" class="main-container">
        <div class="overlay">
            <h2>RALLY ITALIA SARDEGNA</h2>

            <div class="stage-container">    
                <h3>LITTICHEDDA 13.30km</h3> 
                <h5>(FIRST THIRD OF RENA MAJORE STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsLittichedda)): ?>
                ...
                <table class="time-table">
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsLittichedda as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('sardegna_littichedda', 'LITTICHEDDA', '13.30km', 'FIRST THIRD OF RENA MAJORE STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('sardegna_littichedda', 'LITTICHEDDA', '13.30km', 'FIRST THIRD OF RENA MAJORE STAGE')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>BORTIGIADAS 9.02km</h3> 
                <h5>(SECOND THIRD OF RENA MAJORE STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsBortigiadas)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsBortigiadas as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                        <td><?= htmlspecialchars($row["driver"]) ?></td>
                        <td><?= htmlspecialchars($row["class"]) ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('sardegna_bortigiadas', 'BORTIGIADAS', '9.02km', 'SECOND THIRD OF RENA MAJORE STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('sardegna_bortigiadas', 'BORTIGIADAS', '9.02km', 'SECOND THIRD OF RENA MAJORE STAGE')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>MONTE MUVRI 7.51km</h3> 
                <h5>(FINAL QUARTER OF RENA MAJORE STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsMontemuvri)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsMontemuvri as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                        <td><?= htmlspecialchars($row["driver"]) ?></td>
                        <td><?= htmlspecialchars($row["class"]) ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('sardegna_montemuvri', 'MONTE MUVRI', '7.51km', 'FINAL QUARTER OF RENA MAJORE STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('sardegna_montemuvri', 'MONTE MUVRI', '7.51km', 'FINAL QUARTER OF RENA MAJORE STAGE')">SHOW ALL</button>
            </div>
