...
try {
    ...
    $queryVitova = "SELECT * FROM europe_vitova ORDER BY time ASC LIMIT 10;";
    $stmtVitova = $pdo->prepare($queryVitova);
    $stmtVitova->execute();
    $resultsVitova = $stmtVitova->fetchAll(PDO::FETCH_ASSOC);

    $queryLibosvary = "SELECT * FROM europe_libosvary ORDER BY time ASC LIMIT 10;";
    $stmtLibosvary = $pdo->prepare($queryLibosvary);
    $stmtLibosvary->execute();
    $resultsLibosvary = $stmtLibosvary->fetchAll(PDO::FETCH_ASSOC);

    $queryOsicko = "SELECT * FROM europe_osicko ORDER BY time ASC LIMIT 10;";
    $stmtOsicko = $pdo->prepare($queryOsicko);
    $stmtOsicko->execute();
    $resultsOsicko = $stmtOsicko->fetchAll(PDO::FETCH_ASSOC);
...
<html lang="en">
    ...
    <div id="main-container" class="main-container">
        <div class="overlay">
            <h2>CENTRAL EUROPE RALLY</h2>

            <div class="stage-container">    
                <h3>VITOVA 8.77km</h3> 
                <h5>(FIRST THIRD OF ROUSKE STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsVitova)): ?>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('europe_vitova', 'VITOVA', '8.77km', 'FIRST THIRD OF ROUSKE STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('europe_vitova', 'VITOVA', '8.77km', 'FIRST THIRD OF ROUSKE STAGE')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>LIBOSVARY 14.73km</h3> 
                <h5>(MIDDLE SECTION OF ROUSKE STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsLibosvary)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsLibosvary as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                        <td><?= htmlspecialchars($row["driver"]) ?></td>
                        <td><?= htmlspecialchars($row["class"]) ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('europe_libosvary', 'LIBOSVARY', '14.73km', 'MIDDLE SECTION OF ROUSKE STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('europe_libosvary', 'LIBOSVARY', '14.73km', 'MIDDLE SECTION OF ROUSKE STAGE')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>OSICKO 8.94km</h3> 
                <h5>(FINAL THIRD OF ROUSKE STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsOsicko)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('europe_osicko', 'OSICKO', '8.94km', 'FINAL THIRD OF ROUSKE STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('europe_osicko', 'OSICKO', '8.94km', 'FINAL THIRD OF ROUSKE STAGE')">SHOW ALL</button>
            </div>
