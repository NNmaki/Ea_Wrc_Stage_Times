...
try {
    ...
    $queryFridao = "SELECT * FROM portugal_fridao ORDER BY time ASC LIMIT 10;";
    $stmtFridao = $pdo->prepare($queryFridao);
    $stmtFridao->execute();
    $resultsFridao = $stmtFridao->fetchAll(PDO::FETCH_ASSOC);

    $queryTouca = "SELECT * FROM portugal_touca ORDER BY time ASC LIMIT 10;";
    $stmtTouca = $pdo->prepare($queryTouca);
    $stmtTouca->execute();
    $resultsTouca = $stmtTouca->fetchAll(PDO::FETCH_ASSOC);

    $queryCarrazedo = "SELECT * FROM portugal_carrazedo ORDER BY time ASC LIMIT 10;";
    $stmtCarrazedo = $pdo->prepare($queryCarrazedo);
    $stmtCarrazedo->execute();
    $resultsCarrazedo = $stmtCarrazedo->fetchAll(PDO::FETCH_ASSOC);
...
<html lang="en">
    ...
    <div id="main-container" class="main-container">
        <div class="overlay">
            <h2>VODAFONE RALLY DE PORTUGAL</h2>

            <div class="stage-container">    
                <h3>FRIDAO 16.72km</h3> 
                <h5>(FIRST QUARTER OF BAIAO STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsFridao)): ?>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('portugal_fridao', 'FRIDAO', '16.72km', 'FIRST QUARTER OF BAIAO STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('portugal_fridao', 'FRIDAO', '16.72km', 'FIRST QUARTER OF BAIAO STAGE')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>TOUCA 7.51km</h3> 
                <h5>(SECOND QUARTER OF BAIAO STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsTouca)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('portugal_touca', 'TOUCA', '7.51km', 'SECOND QUARTER OF BAIAO STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('portugal_touca', 'TOUCA', '7.51km', 'SECOND QUARTER OF BAIAO STAGE')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>CARRAZEDO 7.48km</h3> 
                <h5>(THIRD QUARTER OF BAIAO STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsCarrazedo)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsCarrazedo as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                        <td><?= htmlspecialchars($row["driver"]) ?></td>
                        <td><?= htmlspecialchars($row["class"]) ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('portugal_carrazedo', 'CARRAZEDO', '7.48km', 'THIRD QUARTER OF BAIAO STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('portugal_carrazedo', 'CARRAZEDO', '7.48km', 'THIRD QUARTER OF BAIAO STAGE')">SHOW ALL</button>
            </div>
