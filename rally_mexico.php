...
try {
    ...
    $queryOrtega = "SELECT * FROM mexico_ortega ORDER BY time ASC LIMIT 10;";
    $stmtOrtega = $pdo->prepare($queryOrtega);
    $stmtOrtega->execute();
    $resultsOrtega = $stmtOrtega->fetchAll(PDO::FETCH_ASSOC);

    $queryIbarrilla = "SELECT * FROM mexico_ibarrilla ORDER BY time ASC LIMIT 10;";
    $stmtIbarrilla = $pdo->prepare($queryIbarrilla);
    $stmtIbarrilla->execute();
    $resultsIbarrilla = $stmtIbarrilla->fetchAll(PDO::FETCH_ASSOC);


    $queryAlfaro = "SELECT * FROM mexico_alfaro ORDER BY time ASC LIMIT 10;";
    $stmtAlfaro = $pdo->prepare($queryAlfaro);
    $stmtAlfaro->execute();
    $resultsAlfaro = $stmtAlfaro->fetchAll(PDO::FETCH_ASSOC);
...
<html lang="en">
    ...
    <div id="main-container" class="main-container">
        <div class="overlay">
            <h2>GUANAJUATO RALLY MEXICO</h2>

            <div class="stage-container">    
                <h3>ORTEGA 13.10km</h3> 
                <h5>(FIRST HALF OF EL CHOCOLATE STAGE)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsOrtega)): ?>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('mexico_ortega', 'ORTEGA', '13.10km', 'FIRST HALF OF EL CHOCOLATE STAGE')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('mexico_ortega', 'ORTEGA', '13.10km', 'FIRST HALF OF EL CHOCOLATE STAGE')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>IBARRILLA 12.92km</h3> 
                <h5>(FIRST HALF OF OTATES STAGE, BUT RUN IN THE REVERSE DIRECTION)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsIbarrilla)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                    ...
                    <tbody>
                        <?php  $rank = 1;
                        foreach ($resultsIbarrilla as $row): ?>
                        <tr>
                        <td><?= $rank ?></td>
                        <td><?= htmlspecialchars($row["driver"]) ?></td>
                        <td><?= htmlspecialchars($row["class"]) ?></td>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('mexico_ibarrilla', 'IBARRILLA', '12.92km', 'FIRST HALF OF OTATES STAGE, BUT RUN IN THE REVERSE DIRECTION')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('mexico_ibarrilla', 'IBARRILLA', '12.92km', 'FIRST HALF OF OTATES STAGE, BUT RUN IN THE REVERSE DIRECTION')">SHOW ALL</button>
            </div>
            ...
            <div class="stage-container">    
                <h3>ALFARO 8.00km</h3> 
                <h5>(STAGE FOLLOWS OTATES ROUTE IN REVERSE BEFORE TURNING NORT PAST THE SAN PEDRO CHURCH)</h5>
                <h5>TOP TEN FASTEST TIMES</h5>
                <?php if (empty($resultsAlfaro)): ?>
                <p>No records at the moment</p>
                <?php else: ?>
                    <table class="time-table">
                    <thead>
                ...
                <?php endif; ?>
                <button class="basic-button" onclick="openNewRecord('mexico_alfaro', 'ALFARO', '8.00', 'STAGE FOLLOWS OTATES ROUTE IN REVERSE BEFORE TURNING NORT PAST THE SAN PEDRO CHURCH')">ADD NEW</button>
                <button class="basic-button" onclick="showAllTimes('mexico_alfaro', 'ALFARO', '8.00', 'STAGE FOLLOWS OTATES ROUTE IN REVERSE BEFORE TURNING NORT PAST THE SAN PEDRO CHURCH')">SHOW ALL</button>
            </div>
